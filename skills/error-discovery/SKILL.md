...
3. Progress view. Two sections:
   - Failure modes: a grid of uniform-sized cards, one per failure mode the agent has categorized so far. Each card shows the mode name, the annotation count as plain text, and the notes. The reviewer can click a note to go to that annotation in the content view.
