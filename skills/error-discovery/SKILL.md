...
3. Progress view. Two sections:
   - Failure modes: a grid of modes the agent has categorized so far. Each card is a failure mode, uniform in size, showing the annotation count as text. Inside each card, list the notes. The reviewer can click a note to go to that annotation in the content view.
