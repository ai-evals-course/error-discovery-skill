...
3. Progress view. Two sections:
   - Failure modes: a grid of uniform-sized cards, one per failure mode the agent has categorized so far. Show the annotation count as plain text on each card, and keep every card the same size: sizing cards by count invites the reviewer to read area as how common a failure is, but discovery counts only reflect where the reviewer happened to look. Inside each card, list the notes. The reviewer can click a note to go to that annotation in the content view.
