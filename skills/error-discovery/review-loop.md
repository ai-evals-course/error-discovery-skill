...
When you discover a new failure mode (or an existing one becomes clearer), scan **all** records for instances. Include both reviewed and unreviewed records. Wait until the human has annotated about 10 distinct records before the first corpus-wide scan; early annotations are too weak a signal of what the human considers a failure.
