...
4. **Monitor the annotations file.** Write a Python polling script that watches `error_discovery_data/annotations.json` for changes every 2 seconds, comparing content to detect new or deleted annotations. Each change emits a line to stdout with the total count, number of records, and the latest note text. Run the script with whatever your environment provides: with a Monitor tool (Claude Code), run it with `persistent: true` so each stdout line becomes a notification and you react in real time; without one (Codex CLI and others), run it in the background with stdout appended to a log file and read the file's new lines whenever you take a turn. Tell the user how you are watching for annotations. Do not use `inotifywait` or filesystem events — poll with a simple read-compare loop for cross-platform reliability.
